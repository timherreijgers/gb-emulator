...
class CpATest : public InstructionTestBase
{
protected:
    void SetUp() override
    {
...
TEST_F(CpATest, ExecutingOpCode)
{
    m_program.WriteProgram({0xBF_b, 0x00_b});

    m_cpu.Registers().accumulator = 0x02_b;

    m_cpu.Step();
    ASSERT_THAT(m_cpu.Registers().instructionRegister, ::testing::Eq(0xBF_b));
...
TEST_F(CpATest, ExecutingOpCode_SetsSubtractionFlag)
{
    m_program.WriteProgram({0xBF_b, 0x00_b});
...
TEST_F(CpATest, ExecutingOpCode_SetsZeroFlagTo1)
{
    m_program.WriteProgram({0xBF_b, 0x00_b});
...
TEST_F(CpATest, ExecutingOpCode_SetsHalfCarryTo0)
{
    m_program.WriteProgram({0xBF_b, 0x00_b});
...
TEST_F(CpATest, ExecutingOpCode_SetsCarryTo0)
{
    m_program.WriteProgram({0xBF_b, 0x00_b});
