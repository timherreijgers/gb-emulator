...
class IncTest : public InstructionTestBase, public ::testing::WithParamInterface<InstructionPair>
{
protected:
    void SetUp() override
    {
...
TEST_P(IncTest, ExecutingOpCode)
{
...
TEST_P(IncTest, ExecutingOpCode_SetsSubtractionFlagTo0)
{
    const auto& [instruction, targetRegister] = GetParam();
    m_program.WriteProgram({instruction, 0x00_b});
...
TEST_P(IncTest, ExecutingOpCode_SetsZeroFlagTo0)
{
    const auto& [instruction, targetRegister] = GetParam();
    m_program.WriteProgram({instruction, 0x00_b});
...
TEST_P(IncTest, ExecutingOpCode_SetsZeroFlagTo1_AndHalfCarryTo1)
{
    const auto& [instruction, targetRegister] = GetParam();
    m_program.WriteProgram({instruction, 0x00_b});
...
TEST_P(IncTest, ExecutingOpCode_SetsHalfCarryTo0)
{
    const auto& [instruction, targetRegister] = GetParam();
    m_program.WriteProgram({instruction, 0x00_b});
...
TEST_P(IncTest, ExecutingOpCode_SetsHalfCarryTo1)
{
    const auto& [instruction, targetRegister] = GetParam();
    m_program.WriteProgram({instruction, 0x00_b});
...
INSTANTIATE_TEST_SUITE_P(IncTest, IncTest,
                         ::testing::Values(
                         ...
                         [](const testing::TestParamInfo<IncTest::ParamType>& info) {
