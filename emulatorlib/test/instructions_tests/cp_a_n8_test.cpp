...
class CpAN8Test : public InstructionTestBase
{
protected:
    void SetUp() override
    {
...
TEST_F(CpAN8Test, ExecutingOpCodeDoesNotModifyAccumulator)
{
    m_program.WriteProgram({0xFE_b, 0x03_b});
    m_cpu.Registers().accumulator = 0x05_b;

    m_cpu.Step();
    ASSERT_THAT(m_cpu.Registers().instructionRegister, ::testing::Eq(0xFE_b));
    m_cpu.Step();
...
TEST_F(CpAN8Test, SetsZeroFlagWithoutBorrowFlags)
{
    m_program.WriteProgram({0xFE_b, 0x02_b});
...
TEST_F(CpAN8Test, SetsHalfCarryAndCarryFlags)
{
    m_program.WriteProgram({0xFE_b, 0x01_b});
