...
{
    // TODO: Extract this out of here, we'll need this more often
    constexpr auto bit = [](const uint8_t bit) {
    ...
    const auto zSign = bit(7) & cpuRegisters.zRegister;
    ...
    cpuRegisters.flags = 0x00_b
        | ((carryPerBit & bit(3)) > 0_b ? CpuFlags::HalfCarry.AsByte() : 0x00_b)
        | ((carryPerBit & bit(7)) > 0_b ? CpuFlags::Carry.AsByte() : 0x00_b);
    co_yield std::monostate{};
