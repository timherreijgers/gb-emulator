...
class OrAN8Test : public InstructionTestBase
{
protected:
    void SetUp() override
    {
...
TEST_F(OrAN8Test, ExecutingOpCodeClearsNonZeroFlags)
{
    m_program.WriteProgram({0xF6_b, 0x0F_b});
    ...
    m_cpu.Step();
    ASSERT_THAT(m_cpu.Registers().instructionRegister, ::testing::Eq(0xF6_b));
    m_cpu.Step();
...
TEST_F(OrAN8Test, SetsZeroFlagAndClearsOtherFlags)
{
    m_program.WriteProgram({0xF6_b, 0x00_b});
