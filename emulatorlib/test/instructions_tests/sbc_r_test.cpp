...
class SbcRTest : public InstructionTestBase, public ::testing::WithParamInterface<InstructionPair>
{
protected:
    void SetUp() override
    {
...
TEST_P(SbcRTest, ExecutingOpCode)
{
...
TEST_P(SbcRTest, ExecutingOpCode_WithCarryFlag)
{
    const auto& [instruction, sourceRegister] = GetParam();
    m_program.WriteProgram({instruction, 0x00_b});
...
TEST_P(SbcRTest, ExecutingOpCode_SetsSubtractionFlag)
{
    const auto& [instruction, sourceRegister] = GetParam();
    m_program.WriteProgram({instruction, 0x00_b});
...
TEST_P(SbcRTest, ExecutingOpCode_SetsZeroFlagTo1)
{
    const auto& [instruction, sourceRegister] = GetParam();
    m_program.WriteProgram({instruction, 0x00_b});
...
TEST_P(SbcRTest, ExecutingOpCode_ClearsZeroFlag)
{
    const auto& [instruction, sourceRegister] = GetParam();
    m_program.WriteProgram({instruction, 0x00_b});
...
TEST_P(SbcRTest, ExecutingOpCode_SetsHalfCarry)
{
    const auto& [instruction, sourceRegister] = GetParam();
    m_program.WriteProgram({instruction, 0x00_b});
...
TEST_P(SbcRTest, ExecutingOpCode_CarryInSetsHalfCarry)
{
    const auto& [instruction, sourceRegister] = GetParam();
    m_program.WriteProgram({instruction, 0x00_b});
...
TEST_P(SbcRTest, ExecutingOpCode_CarryInSetsHalfCarryAndCarry)
{
    const auto& [instruction, sourceRegister] = GetParam();
    m_program.WriteProgram({instruction, 0x00_b});
...
INSTANTIATE_TEST_SUITE_P(SbcRTest, SbcRTest,
                         ::testing::Values(
                             InstructionPair{0x98_b, FunctionWrapper8Bit<RegisterB>()},
                             InstructionPair{0x99_b, FunctionWrapper8Bit<RegisterC>()},
                             InstructionPair{0x9A_b, FunctionWrapper8Bit<RegisterD>()},
                             InstructionPair{0x9B_b, FunctionWrapper8Bit<RegisterE>()},
                             InstructionPair{0x9C_b, FunctionWrapper8Bit<RegisterH>()},
                             InstructionPair{0x9D_b, FunctionWrapper8Bit<RegisterL>()}),
                         [](const testing::TestParamInfo<SbcRTest::ParamType>& info) {
