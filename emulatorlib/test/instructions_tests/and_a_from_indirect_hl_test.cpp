...
class AndAIndirectHlTest : public InstructionTestBase
{
protected:
    void SetUp() override
    {
...
TEST_F(AndAIndirectHlTest, ExecutingOpCode)
{
    m_program.WriteProgram({0xA6_b});

    m_cpu.Registers().accumulator = 0xF0_b;
    m_cpu.Registers().hlRegister = 0x8080;

    m_cpu.Step();
    ASSERT_THAT(m_cpu.Registers().instructionRegister, ::testing::Eq(0xA6_b));
...
TEST_F(AndAIndirectHlTest, ExecutingOpCode_ClearsSubtractionFlag)
{
    m_program.WriteProgram({0xA6_b});
...
TEST_F(AndAIndirectHlTest, ExecutingOpCode_SetsHalfCarry)
{
    m_program.WriteProgram({0xA6_b});
...
TEST_F(AndAIndirectHlTest, ExecutingOpCode_ClearsZeroFlag)
{
    m_program.WriteProgram({0xA6_b});
