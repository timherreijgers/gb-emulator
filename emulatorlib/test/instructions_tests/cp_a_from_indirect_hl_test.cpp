...
class CpAIndirectTest : public InstructionTestBase
{
protected:
    void SetUp() override
    {
...
TEST_F(CpAIndirectTest, ExecutingOpCode)
{
    m_program.WriteProgram({0xBE_b});

    m_cpu.Registers().accumulator = 0x05_b;
    m_cpu.Registers().hlRegister = 0x8080;

    m_cpu.Step();
    ASSERT_THAT(m_cpu.Registers().instructionRegister, ::testing::Eq(0xBE_b));
...
TEST_F(CpAIndirectTest, ExecutingOpCode_SetsSubtractionFlag)
{
    m_program.WriteProgram({0xBE_b});
...
TEST_F(CpAIndirectTest, ExecutingOpCode_SetsZeroFlagTo1)
{
    m_program.WriteProgram({0xBE_b});
...
TEST_F(CpAIndirectTest, ExecutingOpCode_SetsZeroFlagTo0)
{
    m_program.WriteProgram({0xBE_b});
...
TEST_F(CpAIndirectTest, ExecutingOpCode_SetsHalfCarryTo0)
{
    m_program.WriteProgram({0xBE_b});
...
TEST_F(CpAIndirectTest, ExecutingOpCode_SetsHalfCarryTo1)
{
    m_program.WriteProgram({0xBE_b});
...
TEST_F(CpAIndirectTest, ExecutingOpCode_SetsCarryTo0)
{
    m_program.WriteProgram({0xBE_b});
...
TEST_F(CpAIndirectTest, ExecutingOpCode_SetsCarryTo1)
{
    m_program.WriteProgram({0xBE_b});
...
TEST_F(CpAIndirectTest, ExecutingOpCode_SetsHalfCarryAndCarry)
{
    m_program.WriteProgram({0xBE_b});
    EXPECT_CALL(m_addressableMock, ReadFromAddress(0x8080)).Times(1).WillOnce(testing::Return(0x01_b));
