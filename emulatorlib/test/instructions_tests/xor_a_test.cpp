...
class XorATest : public InstructionTestBase
{
protected:
    void SetUp() override
    {
...
TEST_F(XorATest, ExecutingOpCode_SetsZeroFlagAndClearsOtherFlags)
{
    ...
    m_cpu.Step();
    ASSERT_THAT(m_cpu.Registers().instructionRegister, ::testing::Eq(0xAF_b));
    ASSERT_THAT(m_cpu.Registers().accumulator, ::testing::Eq(0xA5_b));
